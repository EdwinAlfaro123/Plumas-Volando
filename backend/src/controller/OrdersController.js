...
orderController.createOrderFromCart = async (req, res) => {
  ...
  try {
    const { items, subtotal, shipping, total, paymentMethod, customerData } =
      req.body;
...
orderController.insertOrder = async (req, res) => {
  try {
    ...
    if (!products || !Array.isArray(products) || products.length === 0) {
      return res
        .status(400)
        .json({ message: "Debe agregar productos al pedido" });
    }
    ...
    for (let i = 0; i < products.length; i++) {
      ...
      if (!price || price <= 0) {
        return res.status(400).json({
          message: `Agrega un precio válido para ${getProductName(
            productFound
          )}`,
        });
      }
...
orderController.getRecentOrders = async (req, res) => {
  try {
    const recentOrders = await ordersModel
      .find()
      .populate(
        "products.productId",
        "name nombre productName unitPrice price"
      )
      .populate("customerId", "name lastName lastname nombre apellido email")
      .sort({ createdAt: -1 })
      .limit(5);
